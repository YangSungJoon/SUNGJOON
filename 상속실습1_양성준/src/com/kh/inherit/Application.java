...
public class Application {
	...
	public static void main(String[] args) {
		...
		/*3명의 학생 정보를 기록할 수 있게 객체 배열을 할당하고 아래의 사용데이터를 
		  참고하여 3명의 학생 객체 생성 후 반복문을 통해 출력 */
		...
		sArry[2] = new Student("강개순", 23, 167.0, 45.0, 4, "정보통신학과");
		...
		/* 1. 최대 10명의 사원 정보를 기록할 수 있게 객체 배열 생성
		 * 2. 키보드로 사원 정보를 입력 받도록 구현 */
		// employee타입을 담을 수 있는 10개의 방이 생성되고
		// 각 방은 참조변수의 초기값인 null로 초기화된다.
		...
		Employee emp = new Employee(name, age, height, weight, salary, dept); // 순서중요
		// 사용자로부터 입력받은 값으로 사원을 생성하고 배열에 저장
		empArry[index] = emp;
		// 사원이 입력된 후 다음번 방을 가리킴
		index++;
		...
		for(Employee e : empArry) {
			...
			// e. <-- 주소접근연산자인 .을 이용할경우 null은 주소를 가지고 있지 않으므로 오류가 발생!
			if(e == null) {
				break;
			}
